...
const states: Record<GenerationTask["status"], string> = { queued: "排队中", running: "生成中", succeeded: "已完成", failed: "未完成", canceled: "已取消" };
...
export default function StudioPage() {
    ...
    const quote = useQuery({ queryKey: ["generation-quote", session, model?.id, count, seconds, capability, conversationID, quotedPrompt], queryFn: ({ signal }) => quoteGeneration({ modelId: model!.id, count: capability === "text" ? 1 : count, content: quotedPrompt, conversationId: capability === "text" ? conversationID || undefined : undefined, parameters: capability === "video" ? { seconds } : {} }, signal), enabled: Boolean(model && quotedPrompt.trim()), gcTime: 0 });
    ...
    const detail = useQuery({ queryKey: ["studio-batch", session, currentBatchID], queryFn: () => getStudioBatch(currentBatchID!), enabled: Boolean(currentBatchID) && capability !== "text", refetchInterval: (query) => query.state.data?.tasks.some((task) => task.status === "queued" || task.status === "running") ? 2500 : false });
    const conversations = useQuery({ queryKey: ["studio-conversations", session], queryFn: listTextConversations, enabled: capability === "text" });
    const conversation = useQuery({ queryKey: ["studio-conversation", session, conversationID], queryFn: () => getTextConversation(conversationID!), enabled: Boolean(conversationID) && capability === "text", refetchInterval: (query) => ["queued", "running"].includes(query.state.data?.latestRequest?.status || "") ? 2500 : false });
    ...
    useEffect(() => { if (previousSession.current !== session) { pendingKey.current = null; setFiles([]); setBatchID(null); setConversationID(null); previousSession.current = session; } }, [session]);
    ...
    const submit = useMutation({ mutationFn: async () => {
        ...
        const input = { modelId: model.id, prompt: prompt.trim(), count: capability === "text" ? 1 : count, capability, size, seconds, voice, conversationID, files: files.map((file) => [file.name, file.size, file.lastModified]) };
        ...
        if (capability === "text") {
            const accepted = await queueTextRequest({ requestId, modelId: model.id, content: input.prompt, conversationId: conversationID || undefined, title: input.prompt.slice(0, 100), attachmentMediaIds: mediaIds });
            assertCurrentSession(version);
    ...
    useEffect(() => {
        if (!latestText || !["queued", "running"].includes(latestText.status) || !conversationID) return;
    ...
    const settledKey = capability === "text"
        ...
        : detail.data?.tasks.length && detail.data.tasks.every((task) => !["queued", "running"].includes(task.status)) ? `${currentBatchID}:${detail.data.tasks.map((task) => task.status).join(",")}` : "";
    useEffect(() => {
    ...
    const busyConversation = latestText && ["queued", "running"].includes(latestText.status);
    ...
    return <div className="flex min-h-full flex-col bg-background text-foreground">
        ...
        <div className="grid flex-1 lg:grid-cols-[370px_minmax(0,1fr)] xl:grid-cols-[400px_minmax(0,1fr)]">
            <section className="border-b border-border p-5 lg:border-b-0 lg:border-r lg:p-7">
                ...
                <Button className="mt-7" size="large" type="primary" block icon={<Play className="size-4" />} loading={submit.isPending} disabled={!model || !prompt.trim() || quotedPrompt !== prompt || !quote.data || quote.isFetching || Boolean(busyConversation)} onClick={() => submit.mutate()}>开始生成{quote.data ? ` · 预冻结 ¥${quote.data.quote.estimatedHold}` : ""}</Button><p className="mt-3 text-xs leading-5 text-muted-foreground">{quote.data?.quote.variable ? capability === "audio" ? "按实际音频时长结算，当前以 5 秒预估冻结。" : "按实际用量结算，预估冻结额包含当前分组折扣。" : "费用已按当前分组价格计算。"}成功后结算，失败自动退回，离开页面后任务继续执行。</p>{quote.error ? <Alert className="mt-3" type="warning" title={quote.error.message} /> : null}
            </section>
            <section className="min-w-0 p-5 lg:p-7">
                <div className="mb-5 flex items-center justify-between"><h2 className="flex items-center gap-2 text-sm font-medium"><Clock3 className="size-4" />{capability === "text" ? "对话与历史" : "生成结果"}</h2>{capability === "text" ? <Button type="text" onClick={() => { setConversationID(null); setSearch({ type: "text" }); }}>新建对话</Button> : <Link to="/canvas" className="text-xs text-muted-foreground">进入无限画布 ↗</Link>}</div>
                {capability === "text" ? <>
                    ...
                    {busyConversation && latestText.partialText ? <article className="mt-6 border-b border-border pb-6"><p className="mb-2 text-xs text-muted-foreground">助手 · 正在生成</p><p className="whitespace-pre-wrap break-words text-sm leading-7">{latestText.partialText}</p></article> : null}
                    {busyConversation ? <div className="mt-5 flex items-center justify-between"><span className="flex items-center gap-2 text-sm text-muted-foreground"><Spin size="small" />正在生成回复</span><Button type="text" icon={<Square className="size-3" />} loading={cancel.isPending} onClick={() => cancel.mutate(latestText.id)}>取消</Button></div> : latestText?.status === "failed" ? <Alert className="mt-5" type="warning" title="本次生成未完成，余额已退回" /> : !conversationID ? <div className="py-20"><Empty description="让第一句话成为创作的起点" /></div> : latestText?.billed ? <p className="mt-5 text-xs text-muted-foreground">本次实付 ¥{latestText.billed}</p> : null}
                </> : <>
