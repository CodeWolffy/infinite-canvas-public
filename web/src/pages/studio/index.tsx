...
export default function StudioPage() {
    ...
    const quote = useQuery({ queryKey: ["generation-quote", session, model?.id, count, seconds, capability, conversationID, quotedPrompt, maxTokens], queryFn: ({ signal }) => quoteGeneration({ modelId: model!.id, count: capability === "text" ? 1 : count, content: quotedPrompt, conversationId: capability === "text" ? conversationID || undefined : undefined, parameters: capability === "video" ? { seconds } : capability === "text" && maxTokens ? { max_tokens: maxTokens } : {} }, signal), enabled: Boolean(model && quotedPrompt.trim() && !(capability === "text" && model.requiresMaxOutputTokens && !maxTokens)), gcTime: 0 });
    ...
    const submit = useMutation({ mutationFn: async () => {
        ...
        if (capability === "text" && model.requiresMaxOutputTokens && !maxTokens) throw new Error("此模型需要填写最大输出 token 数");
        const version = useUserStore.getState().sessionVersion;
        const input = { modelId: model.id, prompt: prompt.trim(), count: capability === "text" ? 1 : count, capability, size, seconds, voice, maxTokens, conversationID, files: files.map((file) => [file.name, file.size, file.lastModified]) };
        ...
        if (capability === "text") {
            const accepted = await queueTextRequest({ requestId, modelId: model.id, content: input.prompt, conversationId: conversationID || undefined, title: input.prompt.slice(0, 100), attachmentMediaIds: mediaIds, parameters: maxTokens ? { max_tokens: maxTokens } : {} });
            assertCurrentSession(version);
    ...
    return <div className="flex min-h-full flex-col bg-background text-foreground">
        ...
        <div className="grid flex-1 lg:grid-cols-[370px_minmax(0,1fr)] xl:grid-cols-[400px_minmax(0,1fr)]">
            <section className="border-b border-border p-5 lg:border-b-0 lg:border-r lg:p-7">
                ...
                {capability === "text" ? <div className="mt-5"><label className="mb-2 block text-sm" htmlFor="studio-max-tokens">最大输出 token{model?.requiresMaxOutputTokens ? "（必填）" : "（选填）"}</label><InputNumber id="studio-max-tokens" min={1} precision={0} className="!w-full" value={maxTokens} onChange={setMaxTokens} placeholder="由你指定，不设置默认值" /><Link to="/text" className="mt-3 inline-block text-xs text-muted-foreground">打开文本对话页 ↗</Link></div> : null}
