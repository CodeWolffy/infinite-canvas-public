...
import { Alert, App, Button, Checkbox, Drawer, Empty, Form, Input, InputNumber, Modal, Select, Space, Switch, Table, Tabs, Tag, Tooltip } from "antd";
...
import { batchSaveModelChannelBindings, createAdminChannel, createAdminModel, deleteAdminChannel, fetchAdminChannelModels, getAdminChannels, getAdminModels, getTaskAdapters, saveModelChannelBinding, updateAdminChannel, type AdminChannel, type ChannelInput } from "@/services/api/admin-platform";
import ChannelMonitoring from "./components/channel-monitoring";
import ChannelKeys from "./components/channel-keys";
import { capabilityLabel, channelProtocolOptions, modelCapabilities, type ModelCapability } from "@/lib/model-capabilities";
...
type QuickModelValues = Omit<ModelSettingsValues, "capability"> & {
...
export default function AdminChannelsPage() {
    ...
    const checkAll = useMutation({ mutationFn: (capability: ModelCapability) => checkAllChannels(capability), onSuccess: (result, capability) => { void refresh(); message.success(result.queued ? `已提交 ${result.queued} 个${capabilityLabel(capability)}渠道检测` : "没有可检测的启用渠道"); }, onError: (error: Error) => message.error(error.message) });
    ...
    const quickModelMutation = useMutation({
        mutationFn: async (values: QuickModelValues) => {
            ...
            if (modelId === createModelValue) {
                if (!values.name || !values.displayName || !values.status) throw new Error("请完善平台模型信息");
                const capability = modelResult.channel.capability;
                const model = await createAdminModel({ name: values.name, displayName: values.displayName, capability, status: values.status, description: null, ...modelSettingsPayload({ ...values, capability }) });
                modelId = model.id;
            }
    ...
    useEffect(() => {
        ...
        form.resetFields();
        form.setFieldsValue(editing ? { name: editing.name, capability: editing.capability, protocol: editing.protocol, baseUrl: editing.baseUrl, status: editing.status, timeoutSeconds: editing.timeoutMs / 1000, maxConcurrency: editing.maxConcurrency, cooldownSeconds: editing.cooldownSeconds ?? 120, apiKeysText: "", keyStrategy: editing.keyStrategy, taskAdapter: editing.taskAdapter } : { capability: category.value, protocol: "openai", status: "disabled", timeoutSeconds: 300, maxConcurrency: 20, cooldownSeconds: 120, keyStrategy: "round_robin", taskAdapter: "" });
    }, [editing, form, category.value]);
    ...
    const categoryChannels = (channelsQuery.data || []).filter((channel) => channel.capability === category.value);
    const visibleChannels = categoryChannels.filter((channel) => `${channel.name} ${channel.baseUrl} ${channel.protocol}`.toLowerCase().includes(keyword.trim().toLowerCase()));
    ...
    const openQuickModel = (upstreamModel: string) => {
        ...
        const matchingModels = matchingPlatformModels.filter((model) => model.name === suggestedName);
        const matchedModel = matchingModels.length === 1 ? matchingModels[0] : undefined;
        quickModelForm.resetFields();
        quickModelForm.setFieldsValue({ targetModelId: matchedModel?.id || createModelValue, name: suggestedName, displayName: suggestedName, status: "draft", priority: 0, weight: 100, enabled: true });
        setConfiguringUpstream(upstreamModel);
    };

    return (
        <div className="w-full px-6 py-6 lg:px-8 lg:py-8">
            <div className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-2xl font-semibold tracking-tight">渠道管理</h1><p className="mt-2 text-sm text-muted-foreground">按类型独立配置接口、密钥与调度，同一服务商可分别创建不同类型的渠道。</p></div><Space wrap><Link to={`/admin/models?type=${category.value}`}><Button>{category.label}模型</Button></Link><Button loading={checkAll.isPending} onClick={() => modal.confirm({ title: `检测全部${category.label}渠道？`, content: `仅提交已启用且配置了检测项目的${category.label}渠道。生成检测会产生真实上游费用。`, okText: "开始检测", onOk: () => checkAll.mutateAsync(category.value) })}>检测{category.label}渠道</Button><Button type="primary" icon={<Plus className="size-4" />} onClick={() => setEditing(null)}>创建{category.label}渠道</Button></Space></div>
            <Tabs className="mt-6" activeKey={category.value} onChange={(value) => { setSearchParams({ type: value }); setKeyword(""); }} items={modelCapabilities.map(({ value, label, icon: Icon }) => ({ key: value, label: <span className="inline-flex items-center gap-2"><Icon className="size-4" />{label}<span className="text-xs tabular-nums text-muted-foreground">{(channelsQuery.data || []).filter((channel) => channel.capability === value).length}</span></span> }))} />
            <div className="mb-5 flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-medium">{category.label}渠道</h2><p className="mt-1 text-sm text-muted-foreground">仅绑定{category.label}模型，密钥与调度参数独立生效。</p></div><Input className="!w-full sm:!w-72" prefix={<Search className="size-4 text-muted-foreground" />} allowClear placeholder={`搜索${category.label}渠道名称、协议或地址`} value={keyword} onChange={(event) => setKeyword(event.target.value)} /></div>
            ...
            <Modal title={`${editing ? "编辑" : "创建"}${capabilityLabel(formCapability)}渠道`} open={editing !== undefined} footer={null} onCancel={() => setEditing(undefined)} destroyOnHidden width={600}>
                <Form<ChannelValues> form={form} layout="vertical" requiredMark={false} className="pt-3" onFinish={(values) => saveMutation.mutate(values)}>
                    ...
                    {formCapability === "video" ? <Form.Item name="taskAdapter" label="视频任务适配器" extra="按接口协议选择已接入的视频生成方式。"><Select loading={adaptersQuery.isLoading} options={[{ value: "", label: "按渠道协议自动选择" }, ...(adaptersQuery.data?.adapters || []).filter((adapter) => adapter.capability === "video" && adapter.protocol === protocol).map((adapter) => ({ value: adapter.id, label: adapter.name }))]} /></Form.Item> : null}
                    {formCapability === "video" && adaptersQuery.error ? <Alert className="mb-4" type="error" showIcon title={adaptersQuery.error.message} /> : null}
                    <div className="grid grid-cols-3 gap-4">
                        <Form.Item name="timeoutSeconds" label="超时（秒）" extra={formCapability === "video" ? "按上游视频生成耗时配置。" : undefined} rules={[{ required: true }]}><InputNumber className="!w-full" min={1} precision={0} /></Form.Item>
            ...
            <Modal title={`批量绑定平台模型 (${selectedModels.length} 个模型)`} open={batchOpen} footer={null} onCancel={() => setBatchOpen(false)} destroyOnHidden width={560}>
                ...
                <Form form={batchForm} layout="vertical" requiredMark={false} onFinish={(values) => batchMutation.mutate(values)}>
                    <Form.Item name="targetModelId" label="目标平台模型" rules={[{ required: true, message: "请选择平台模型" }]}>
                        <Select showSearch={{ optionFilterProp: "label" }} loading={modelsQuery.isLoading} placeholder="请选择同类型的平台模型" notFoundContent="暂无同类型模型，请先创建" options={matchingPlatformModels.map((model) => ({ value: model.id, label: `${model.displayName} · ${model.name}` }))} />
                    </Form.Item>
            ...
            <Modal title={`配置${modelResult ? capabilityLabel(modelResult.channel.capability) : ""}模型`} open={Boolean(configuringUpstream)} footer={null} onCancel={() => setConfiguringUpstream(null)} destroyOnHidden width={560}>
                ...
                <Form<QuickModelValues> form={quickModelForm} layout="vertical" requiredMark={false} onFinish={(values) => quickModelMutation.mutate(values)}>
                    ...
                    <Form.Item name="targetModelId" label="平台模型" extra="仅可绑定同类型模型；唯一同名模型会自动选中，也可以创建新模型。" rules={[{ required: true, message: "请选择平台模型" }]}><Select showSearch={{ optionFilterProp: "label" }} loading={modelsQuery.isLoading} options={[{ value: createModelValue, label: "＋ 创建同类型平台模型" }, ...matchingPlatformModels.map((model) => ({ value: model.id, label: `${model.displayName} · ${model.name}` }))]} /></Form.Item>
                    <Form.Item noStyle shouldUpdate={(previous, current) => previous.targetModelId !== current.targetModelId}>{({ getFieldValue }) => getFieldValue("targetModelId") === createModelValue ? <>
                        ...
                        <Form.Item name="status" label="发布状态" rules={[{ required: true }]}><Select options={[{ value: "draft", label: "草稿" }, { value: "published", label: "已发布" }, { value: "disabled", label: "已停用" }]} /></Form.Item>
                        <ModelSettingsFields capability={modelResult?.channel.capability || category.value} />
                    </> : null}</Form.Item>
...
function channelPayload(values: ChannelValues) {
    ...
    return { ...rest, taskAdapter: values.capability === "video" ? values.taskAdapter || "" : "", timeoutMs: timeoutSeconds * 1000, apiKeys: (apiKeysText || "").split(/\r?\n/).map((key) => key.trim()).filter(Boolean) };
}
