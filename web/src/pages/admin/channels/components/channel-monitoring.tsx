...
export default function ChannelMonitoring({ channel, onClose }: { channel: AdminChannel; onClose: () => void }) {
    ...
    useEffect(() => { const value = channel.monitoring; form.setFieldsValue({ intervalMinutes: value?.intervalMinutes || 0, autoDisableAfter: value?.autoDisableAfter ?? 3, bindingIds: value?.bindingIds || [], prompt: value?.prompt || "", parametersText: JSON.stringify(value?.parameters || {}, null, 2), checkModels: value?.checkModels ?? true, balanceThreshold: value?.balanceThreshold ?? null }); }, [channel, form]);
    ...
    const save = useMutation({ mutationFn: (values: Values) => {
        ...
        if (channel.protocol === "anthropic" && values.bindingIds?.length && !["max_tokens", "max_completion_tokens", "maxOutputTokens"].some((key) => Number((parameters as Record<string, unknown>)[key]) > 0)) throw new Error("Claude 生成检测必须在参数中填写最大输出 token 数，例如 max_tokens");
    ...
    return <Drawer title={`${channel.name} · 主动检测`} open onClose={onClose} size={680}>
        ...
        <Form form={form} layout="vertical" onFinish={(values) => save.mutate(values)}>
            ...
            <Form.Item name="autoDisableAfter" label="连续检测失败后自动停用" extra="默认 3 次，0 表示关闭；沿用渠道冷却时间，下一次有效检测成功后恢复。手动停用的渠道不会自动启用。"><InputNumber min={0} precision={0} className="!w-full" /></Form.Item>
            ...
            {current.autoDisabledAt ? <Alert className="mb-5" type="warning" showIcon title={`已自动停用 · 连续 ${current.consecutiveCheckFailures} 次检测失败`} description="冷却结束后按已保存的检测间隔检查；也可手动提交检测。" /> : null}
