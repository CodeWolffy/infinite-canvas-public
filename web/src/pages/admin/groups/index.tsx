...
export default function AdminGroupsPage() {
    ...
    const policy = useMutation({ mutationFn: (values: GroupPolicy & { restricted: boolean; storageQuotaMb: number }) => saveGroupPolicy(policyGroup!.id, { modelIds: values.restricted ? values.modelIds || [] : null, storageQuotaBytes: Math.round(Number(values.storageQuotaMb || 0) * 1024 * 1024) }), onSuccess: () => { void refresh(); setPolicyGroup(null); message.success("模型权限与存储配额已保存"); }, onError: (error: Error) => message.error(error.message) });
    ...
    const columns: TableColumnsType<UserGroup> = [
        ...
        { title: "模型与存储", key: "policy", render: (_, group) => <div className="space-y-1"><div className="text-sm">{group.modelIds == null ? "不限模型" : `可用 ${group.modelIds.length} 个模型`}</div><div className="text-xs text-muted-foreground">{Number(group.storageQuotaBytes || 0) > 0 ? `存储 ${formatBytes(Number(group.storageQuotaBytes))}` : "未设存储配额"}</div><Button type="link" size="small" onClick={() => { setPolicyGroup(group); policyForm.setFieldsValue({ restricted: group.modelIds != null, modelIds: group.modelIds || [], storageQuotaMb: Number(group.storageQuotaBytes || 0) / (1024 * 1024) }); }}>权限与存储</Button></div> },
    ...
    return <div className="w-full px-5 py-8 lg:px-8">
        <div className="mb-7 flex flex-wrap items-end justify-between gap-4"><div><Users className="mb-3 size-5 text-muted-foreground" /><h1 className="text-2xl font-semibold">用户分组</h1><p className="mt-2 text-sm text-muted-foreground">配置分组折扣、模型权限与存储配额。余额永久有效，额外赠送通过兑换码发放。</p></div><Button type="primary" icon={<Plus className="size-4" />} onClick={() => { setEditing(null); form.setFieldsValue({ discount: 1 }); }}>创建分组</Button></div>
        ...
        <Modal title={`${policyGroup?.name || "分组"} · 模型权限与存储`} open={Boolean(policyGroup)} footer={null} onCancel={() => setPolicyGroup(null)} forceRender>
