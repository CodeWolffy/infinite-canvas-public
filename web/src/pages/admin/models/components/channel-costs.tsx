...
import { saveChannelCost, type CostConfig } from "@/services/api/platform-operations";
...
export default function ChannelCosts({ model }: { model: AdminModel }) {
    ...
    const save = useMutation({ mutationFn: (values: Values) => {
        ...
        return saveChannelCost(model.id, editing!.channelId, cost);
    ...
    return <section className="mt-8 border-t border-border pt-6"><h3 className="font-medium">上游成本配置</h3><p className="mb-4 mt-2 text-sm leading-6 text-muted-foreground">与用户售价独立。使用人民币配置；调用时保存快照，之后可按渠道账单核对。未配置的成本显示为未知。</p>
        <Table rowKey="channelId" size="small" pagination={false} dataSource={bindings.data || []} columns={[{ title: "渠道", dataIndex: "channelName" }, { title: "配置", render: (_, binding) => Object.keys(binding.costConfig || {}).length ? "已配置" : "未知" }, { title: "操作", render: (_, binding) => <Button type="text" size="small" onClick={() => { const config = binding.costConfig || {}; setEditing(binding); form.resetFields(); form.setFieldsValue({ ...config, mode: config.input != null ? "token" : config.second != null ? "second" : config.fixed != null ? "fixed" : "unknown" }); }}>设置成本</Button> }]} />
        <Modal title={`${editing?.channelName || "渠道"} · 人民币成本`} open={Boolean(editing)} onCancel={() => setEditing(null)} footer={null} forceRender>
