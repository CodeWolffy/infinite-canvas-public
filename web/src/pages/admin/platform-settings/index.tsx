...
export default function PlatformSettingsPage() {
    ...
    return <div className="mx-auto w-full max-w-5xl px-5 py-8 lg:px-8">
        ...
        {query.error ? <Alert type="error" title={query.error.message} action={<Button onClick={() => void query.refetch()}>重试</Button>} /> : query.isPending ? <Spin /> : <Form<PlatformSettings> form={form} layout="vertical" onFinish={(values) => mutation.mutate({ ...values, rewardMin: String(values.rewardMin), rewardMax: String(values.rewardMax), referralReward: String(values.referralReward) })}>
            ...
            <div className="grid gap-8 border-t border-border py-7 md:grid-cols-[220px_1fr]"><div><h2 className="font-medium">邀请奖励</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">默认关闭。好友通过推荐链接并使用有效注册邀请码完成注册后，一次性向邀请人入账，余额永久有效。</p></div><div><Form.Item name="referralEnabled" label="开启自动邀请奖励" valuePropName="checked"><Switch /></Form.Item><Form.Item name="referralReward" label="每位受邀新用户奖励（元）" extra="只影响今后的注册，不补发历史奖励；其他赠送使用兑换码。" rules={[{ required: true }]}><InputNumber<string> stringMode min="0" precision={6} className="!w-full" /></Form.Item></div></div>
