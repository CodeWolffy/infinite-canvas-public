...
export default function StatusPage() {
    ...
    const query = useQuery({ queryKey: ["public-model-status", session], queryFn: getPublicStatus, retry: false });
    const models = query.data?.models || [];
    return <main className="h-full overflow-y-auto px-5 py-8 sm:px-8">
        <div className="mx-auto max-w-5xl">
            <section className="pb-9 pt-3 sm:pt-6"><Activity className="mb-5 size-6 text-muted-foreground" /><h1 className="text-3xl font-semibold tracking-tight sm:text-4xl">模型运行状态</h1><p className="mt-4 max-w-2xl text-sm leading-7 text-muted-foreground">这里展示各模型最近一次生成检测结果。未检测或自动检测未及时更新的模型显示为「待检测」，实际请求仍可能受上游波动影响。</p></section>
