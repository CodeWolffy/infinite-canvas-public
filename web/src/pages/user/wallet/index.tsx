...
import GroupGrant from "./components/group-grant";
...
export default function WalletPage() {
    ...
    return <div className="mx-auto w-full max-w-6xl px-5 py-8 lg:px-10 lg:py-10">
        ...
        <GroupGrant />
        {summary ? <div className="mb-8 grid gap-6 border-b border-border pb-8 sm:grid-cols-2 lg:grid-cols-4">{[["本月实付", summary.spent], ["本月充值", summary.recharge], ["本月公益额度", summary.grants], ["本月签到", summary.checkin]].map(([label, value]) => <div key={label}><p className="text-xs text-muted-foreground">{label}</p><p className="mt-2 font-mono text-xl">¥{value}</p></div>)}{summary.spendLimit ? <div className="sm:col-span-2"><p className="text-xs text-muted-foreground">当前周期消费上限</p><p className="mt-2 text-sm text-muted-foreground">已用 ¥{summary.periodSpent}，冻结 ¥{summary.periodFrozen}，上限 ¥{summary.spendLimit}</p></div> : null}</div> : null}
        <Tabs items={[
