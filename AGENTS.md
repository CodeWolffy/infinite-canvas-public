...
- 前端服务请求统一放在 `web/src/services/api/`；本公益平台的图片、视频、文本、音频、画布、素材和账务统一通过 `server/` Go 后端处理，上游渠道密钥不得下发浏览器；`api/` 仅保留为公司版接口参考，不再作为部署入口。
...
- 用户余额、签到、邀请奖励和兑换码到账金额永久保留，不增加按日/周/月重置、到期回收、周期赠送或周期消费额度；额外赠送由管理员通过兑换码发放，自动邀请奖励默认关闭、由管理员显式配置。
- 邀请返利仅在受邀好友真实充值到账时，按实际支付金额与到账时管理员配置的百分比计算；注册只记录关系，签到和兑换码不触发返利，重复支付通知不得重复返利。
- 文本模型最大输出 token 由管理员按模型配置，报价与生成统一从后端读取，用户页面不展示或编辑该数值；模型价格按有效小数位显示，不补多余的零。
- 文本模型可用思考强度由管理员按模型配置，用户端选项与后端校验共用该配置；不要假定所有模型或渠道都支持相同档位。
- 平台文本上下文只沿用当前对话的历史消息，以 `conversationId` 隔离；不自动合并其他对话或增加跨对话的用户画像、长期记忆。
- PowerShell 下使用 `rg` 按文件类型搜索时，传目录并通过 `-g '*.ts'` 等 glob 参数筛选；不要将未展开的 `目录/*.ts` 作为文件路径传入。

- 新增或调整超时、重试次数、大小限制、并发上限等会改变实际行为的边界值前，必须先向用户说明适用环节、默认值和失败后的处理方式，并取得确认；不要把经验值当成纯内部实现静默加入。
- 本公益平台的画布、素材、生成记录、文本消息和人民币账务以 PostgreSQL、MinIO 为权威存储；浏览器缓存和异步回调必须按登录会话隔离。文件/剪贴板读取、解压、图片处理等异步准备开始时就记录会话版本，在准备完成、发请求和写回状态前核对；不要只在请求函数内部才记录版本。
- 金额以整数微元存储，支付金额精确到分；任务创建与余额冻结、结果持久化与结算、失败退款均须在数据库事务中完成。支付回调必须验签并核对商户、订单、支付方式、币种和金额，幂等入账；浏览器回跳不得作为到账依据。
- Go 公益平台使用独立空数据库；启动不得覆盖公司版数据库。Redis 仅用于跨实例频控与渠道槽位，不能作为余额或任务记录的权威存储。
- 已有 Go 数据库的版本升级必须覆盖非空旧库；新增必填列先填充已有记录再收紧非空约束，类型拆分须保留渠道密钥、模型绑定、成本配置和账务，不能只用空库启动验证升级成功。
- 当前部署为公网域名 HTTPS，经 1Panel -> Web Nginx -> API 两层反代，3000/3001 不向公网开放；不得再按仅局域网部署的假设判断安全要求。
- 平台 AI API Key 由后端加密保存和调用；Agent 继续作为每位用户电脑上的可选本地 Codex 插件，平台文本渠道保持独立，不在平台服务器托管 Codex。
