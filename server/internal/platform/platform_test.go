...
func testApp(t *testing.T) *App {
	...
	for _, migration := range []string{"../../migrations/002_token_pricing.sql", "../../migrations/003_platform_features.sql", "../../migrations/004_platform_operations.sql", "../../migrations/005_platform_ops.sql", "../../migrations/006_storage_quota.sql", "../../migrations/007_monitor_cost_link.sql", "../../migrations/008_multi_upstream_models.sql"} {
...
func TestTokenPricingSettlesByUsage(t *testing.T) {
	...
	for _, query := range []struct {
		sql  string
		args []any
	}{{"INSERT INTO models(id,name,display_name,capability,status,price_micros,input_price_per_million,cached_price_per_million,output_price_per_million) VALUES($1,'gpt-test','测试文本','text','published',10000,2000000,200000,4000000)", []any{modelID}}, {"INSERT INTO channels(id,name,protocol,base_url,encrypted_api_key,status) VALUES($1,'测试渠道','openai',$2,$3,'active')", []any{channelID, upstream.URL + "/v1", sealed}}, {"INSERT INTO model_channels(model_id,channel_id,upstream_model) VALUES($1,$2,'gpt-test')", []any{modelID, channelID}}} {
		if _, err := a.DB.Exec(ctx, query.sql, query.args...); err != nil {
			t.Fatal(err)
		}
	}
